...
export const TeacherForm = ({
  ...
  const grades = ['Kindergarten', '1st Grade', '2nd Grade', '3rd Grade', '4th Grade', '5th Grade', '6th Grade', '7th Grade', '8th Grade', '9th Grade', '10th Grade', '11th Grade', '12th Grade', 'Multiple Grades'];
  return <form onSubmit={onSubmit} className="w-full">
      <div className="flex items-center mb-4">
        <button type="button" onClick={onBack} className="mr-2">
          <ArrowLeftIcon size={16} />
        </button>
        <h3 className="text-xl font-semibold">Teacher Signup</h3>
      </div>
      ...
      <div className="mb-4">
        <select name="grade" value={formData.grade} onChange={onChange} required className="w-full p-2 border rounded focus:outline-none focus:ring-2 focus:ring-[#1c3663]">
          <option value="">Select Grade You Teach</option>
          {grades.map(grade => <option key={grade} value={grade}>
              {grade}
            </option>)}
        </select>
      </div>
      <FormField name="className" value={formData.className} onChange={onChange} placeholder="Class Name/Class Code" required />
